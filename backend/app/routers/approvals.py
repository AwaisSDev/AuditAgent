...
@router.post("/approvals/request", response_model=ApprovalRequestOut)
async def request_approval(
    ...
    ws = db.table("workspaces").select("slack_channel_id, notify_email").eq("id", auth.workspace_id).single().execute().data
    ...
    if ws.get("slack_channel_id"):
        ...
        if posted:
            channel, ts = posted
            db.table("approvals").update({"slack_channel": channel, "slack_message_ts": ts}).eq("id", row["id"]).execute()
