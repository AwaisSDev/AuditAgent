...
async def list_questionnaires(workspace_id: str, user: CurrentUser = Depends(require_workspace_member)) -> list[QuestionnaireOut]:
    ...
    return (
        await run_db(
            lambda: db.table("questionnaires").select("*").eq("workspace_id", workspace_id).order("created_at", desc=True).execute()
        )
    ).data
...
async def upload_questionnaire(
    ...
    await run_db(
        lambda: db.storage.from_("questionnaires").upload(
...
async def list_answers(
    ...
    result = []
    for a in answers:
        links = (await run_db(lambda aid=a["id"]: db.table("evidence_links").select("event_id").eq("answer_id", aid).execute())).data
...
async def update_answer(
    ...
    links = (await run_db(lambda: db.table("evidence_links").select("event_id").eq("answer_id", answer_id).execute())).data
...
async def export_docx(workspace_id: str, questionnaire_id: str, user: CurrentUser = Depends(require_workspace_member)) -> StreamingResponse:
    db = get_db()
    ws = (await run_db(lambda: db.table("workspaces").select("name, plan").eq("id", workspace_id).single().execute())).data
    q = (await run_db(lambda: db.table("questionnaires").select("filename").eq("id", questionnaire_id).single().execute())).data
